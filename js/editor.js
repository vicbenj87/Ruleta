...
(function () {
  ...
  function volcarCanciones(canciones) {
    crearModelo();
    ...
    canciones.forEach(function (c) {
      ...
      if (id >= 1 && id <= MAX_FILAS && !filaTieneContenido(filas[id - 1])) {
        ...
        if (siguienteLibre >= MAX_FILAS) { descartadas++; return; }
        destino = siguienteLibre;
      }

      var f = filas[destino];
      f.titulo = String(c.titulo || '').trim();
      var tipo = parseInt(c.tipo, 10);
      f.tipo = (tipo === 1 || tipo === 2 || tipo === 3) ? tipo : 1;
      (Array.isArray(c.diapositivas) ? c.diapositivas : [])
        .slice(0, MAX_DIAPOS)
        .forEach(function (d, j) { f.diapositivas[j] = String(d == null ? '' : d); });
    });
  ...
  function conectarEventos() {
    ...
    dom.confirmarImportar.addEventListener('click', function () {
      try {
        ...
        var descartadas = volcarCanciones(canciones);
        cerrarDialogo(dom.dlgImportar);
        dom.avisoBorradorTexto.textContent = 'Importadas ' + (canciones.length - descartadas) + ' canciones.' +
          (descartadas ? ' ' + descartadas + ' no cabían en las ' + MAX_FILAS + ' filas y se han descartado.' : '');
        dom.avisoBorrador.setAttribute('data-visible', 'true');
      } catch (e) {
